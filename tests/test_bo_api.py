...
class TestApi:
    def test_search_org(self):
        result = bo_api.search_organization(bo_exist_inns[0])
        assert isinstance(result, dict)

    def test_get_org_bfo_details(self):
        result = bo_api.get_organization_bfo_details(bfo_exist_ids[0])
        assert isinstance(result, list)

    def test_get_org_summary(self):
        result = bo_api.get_organization_summary(bfo_exist_ids[0])
        assert isinstance(result, dict)
